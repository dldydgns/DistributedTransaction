...
public class ExchangeService {
    ...
    public ResponseEntity<String> requestExchange(ExchangeRequestDTO requestDTO) {
        try {
            ...
            e.printStackTrace();
            return ResponseEntity.status(500).body("환전요청 생성 실패: " + e.getClass().getSimpleName());
        }
        ...
        try {
            ...
            e.printStackTrace();
            producer.sendCompTx(compServerName + "Deposit", compTransJson);
            return ResponseEntity.status(500).body("입금 요청 중 오류 발생: " + e.getClass().getSimpleName());
        }
    }
