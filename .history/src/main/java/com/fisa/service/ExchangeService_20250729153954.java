...
public class ExchangeService {
    ...
    public ResponseEntity<String> requestExchange(ExchangeRequestDTO request) {
        ...
        try {
            ...
            String quoteUrl = (jsonNode.get("quote").asText().equals("KRW") ? wonhwa : oehwa);
            ...
            if (response.getStatusCode() == HttpStatus.OK) {
                ...
                if(response.getStatusCode() == HttpStatus.OK) {
                    ...
                    // 출금 취소 보상 트랜잭션 생성
                    DepositDTO compTransDTO = new DepositDTO();
                    compTransDTO.setUserid(withdrawalDTO.getUserid());
                    compTransDTO.setAccountid(withdrawalDTO.getAccountid());
                    compTransDTO.setAmount(withdrawalDTO.getAmount());
                    compTransDTO.setDate(withdrawalDTO.getDate());

                    depositJson = objectMapper.writeValueAsString(compTransDTO);
                    response = postJson(baseUrl, depositJson);
