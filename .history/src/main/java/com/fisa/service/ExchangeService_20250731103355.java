...
public class ExchangeService {
        ...
        try {
            ...
            if(checkResponse.getStatusCode().is2xxSuccessful()) {
                try {
                    ...
                    producer.sendCompTx(withdrawalUrl, compTransDTO);
                    exchangeStatusRepository.save(new ExchangeStatus(requestEntity, "출금 취소"));
                } catch(Exception ex) {
                    saveFailToFile(withdrawalDTO, "compensation_fail", requestDTO.getGuid());
                }
            } else {
    ...
    private void saveFailToFile(Object dto, String prefix, UUID guid) {
        try {
            String json = objectMapper.writeValueAsString(dto);
            String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss_SSS"));
            String filename = String.format("/tmp/failed_compensations/%s_%s_%s.json", prefix, guid, timestamp);
    
            File dir = new File("/tmp/failed_compensations");
            if (!dir.exists()) {
                dir.mkdirs();
            }
            ...
            try (FileWriter writer = new FileWriter(filename)) {
                writer.write(json);
                log.info("보상 트랜잭션 JSON 파일 저장 완료: {}", filename);
            }
        } catch (Exception e) {
            log.error("보상 트랜잭션 JSON 파일 저장 실패: GUID = {}, Exception = {}", guid, e.getMessage(), e);
        }
    }
}
