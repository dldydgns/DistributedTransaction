...
public class ExchangeService {
    ...
    public ResponseEntity<String> requestExchange(ExchangeRequestDTO request) {
        try {
            ...
            // todo: 출금요청 DTO로 json생성
            JsonNode original = objectMapper.readTree(reqJson);

            // 새로운 ObjectNode 생성
            ObjectNode extracted = objectMapper.createObjectNode();

            // 필요한 필드만 추출해서 구성
            extracted.put("base", original.get("base").asText());
            extracted.put("base_account", original.get("base_account").asInt());
            extracted.put("base_amount", original.get("base_amount").asInt());
            extracted.put("guid", original.get("guid").asText());
