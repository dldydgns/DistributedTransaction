...
public class ExchangeService {
    ...
    public ResponseEntity<String> requestExchange(ExchangeRequestDTO requestDTO) {
        try {
            ...
            // 입금 로직 전체를 별도 try-catch로 감싸서, 실패 시 보상 실행
            try {
                ...
                if (depositResponse.getStatusCode().is2xxSuccessful()) {
                    ExchangeRequest entity = modelMapper.map(requestDTO, ExchangeRequest.class);
                    exchangeRequestRepository.save(entity);
                    return ResponseEntity.ok("환전에 성공했습니다.");
                } else {
                    // 입금 실패 → 출금 보상
                    sendCompTxForWithdraw(compServerName, withdrawalDTO);
                    return ResponseEntity.status(500).body("입금에 실패했습니다. 출금 금액은 복구됩니다.");
                }

            } catch (Exception depositEx) {
                // 입금 중 어떤 예외가 발생해도 보상 보장
                producer.sendCompTx(compServerName, withdrawalDTO);
                return ResponseEntity.status(500).body("입금 처리 중 예외 발생. 출금 금액은 복구됩니다.");
            }
