...
public class ExchangeService {
    ...
    final String wonhwa = "http://192.168.0.41:8080/api/wonhwa";
    final String oehwa = "http://192.168.0.41:8082/api/oehwa";
    ...
    public ResponseEntity<String> requestExchange(ExchangeRequestDTO requestDTO) {
        ...
        requestDTO.setGuid(UUID.randomUUID());
    
        // 환전 기록 DB 생성
        try {
            requestEntity = modelMapper.map(requestDTO, ExchangeRequest.class);
            requestEntity.setStatus("Fail");
            ...
            return ResponseEntity.status(299).body("환전요청 DB 저장 실패: " + e.getClass().getSimpleName());
        }
        ...
        try {
            ...
            if (withdrawalResponse.getStatusCode().equals(HttpStatus.OK)) {
            ...
            if(checkResponse.getStatusCode().equals(HttpStatus.OK)) {
                ...
                producer.sendCompTx(compServerName + "Deposit", compTransDTO);
            ...
            return ResponseEntity.status(299).body("출금 요청 중 오류 발생: " + e.getClass().getSimpleName());
        }
        ...
        try {
            ...
            if (depositResponse.getStatusCode().equals(HttpStatus.OK)) {
                exchangeStatusRepository.save(new ExchangeStatus(requestEntity, "입금 성공"));
                requestEntity.setStatus("Success");
            ...
            // 입금결과 재확인 결과에 따라 보상트랜잭션 발행
            ResponseEntity<String> checkResponse = checkHistoryByGuid(withdrawalUrl + "/check/deposit", requestDTO.getGuid());
        
            if(checkResponse.getStatusCode().equals(HttpStatus.OK)) {
                exchangeStatusRepository.save(new ExchangeStatus(requestEntity, "입금 성공 확인"));
                requestEntity.setStatus("Success");
            } else {
                exchangeStatusRepository.save(new ExchangeStatus(requestEntity, "입금 실패 확인"));
                DepositDTO compTransDTO = modelMapper.map(withdrawalDTO, DepositDTO.class);
                producer.sendCompTx(compServerName + "Deposit", compTransDTO);
                exchangeStatusRepository.save(new ExchangeStatus(requestEntity, "출금 취소"));
            }

            return ResponseEntity.status(299).body("입금 요청 중 예외 발생: " + e.getClass().getSimpleName());
        }
    }
    ...
    public ResponseEntity<String> checkHistoryByGuid(String url, UUID guid) {
        ...
        String jsonBody = String.format("{\"guid\":\"%s\"}", guid.toString());
