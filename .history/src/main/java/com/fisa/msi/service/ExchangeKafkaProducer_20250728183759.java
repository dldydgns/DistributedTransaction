...
public class ExchangeKafkaProducer {
    ...
    public void sendCompTx(String TOPIC, String message) {
        try {
            ...
            exchangeRequest.setUser_id(userid);
            exchangeRequest.setDirection(direction);
            exchangeRequest.setBase(base);
            exchangeRequest.setQuote(quote);
            exchangeRequest.setBase_amount(base_amount);
            exchangeRequest.setQuote_amount(quote_amount);
            exchangeRequest.setRate(rate);
            exchangeRequest.setGuid(guid); // 엔티티에서 UUID 타입으로 선언돼야 함
